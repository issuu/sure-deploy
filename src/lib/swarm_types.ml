...
module Swarm : sig
  ...
  val get : t -> Uri.t -> (Cohttp.Response.t * Cohttp_async.Body.t) Async_kernel.Deferred.t
end = struct
  type t = {
  ...
  let uri t path =
    Uri.make ~scheme:(get_swarm_scheme t.ssl_config) ~host:t.host ~port:t.port ~path ()

  let get t uri =
    Client.get ?ssl_config:t.ssl_config uri
end
